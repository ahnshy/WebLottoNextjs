...
import { Box, Drawer, List, ListItemButton, ListItemIcon, ListItemText, Collapse, Divider, useMediaQuery, IconButton, Tooltip, Typography } from '@mui/material';
...
export default function Sidebar({ open, onClose, collapsed, setCollapsed }:{ open:boolean; onClose:()=>void; collapsed:boolean; setCollapsed:(v:boolean)=>void }){
      ...
      <List subheader={null} sx={{ pt: 0, flex:1 }}>
        ...
        <Collapse in={open1 || collapsed} timeout="auto" unmountOnExit={!collapsed}>
          <List component="div" disablePadding>
            <ListItemButton selected={section==='당첨번호보기'} sx={{ pl: collapsed ? 2 : 7 }} onClick={()=>{setSection('당첨번호보기'); if(!isMdUp) onClose();}}>
              <ListItemIcon><NumbersIcon/></ListItemIcon>
              <ItemText primary="당첨번호보기" />
            </ListItemButton>
            <ListItemButton selected={section==='당첨 패턴 분석'} sx={{ pl: collapsed ? 2 : 7 }} onClick={()=>{setSection('당첨 패턴 분석'); if(!isMdUp) onClose();}}>
        ...
        <Collapse in={open2 || collapsed} timeout="auto" unmountOnExit={!collapsed}>
          <List component="div" disablePadding>
            <ListItemButton selected={section==='예상번호 추출'} sx={{ pl: collapsed ? 2 : 7 }} onClick={()=>{setSection('예상번호 추출'); if(!isMdUp) onClose();}}>
              <ListItemIcon><AutoAwesomeIcon/></ListItemIcon>
        ...
        <Divider sx={{ my: 1 }} />
        <ListItemButton selected={section==='동기화'} onClick={()=>{setSection('동기화'); if(!isMdUp) onClose();}}>
